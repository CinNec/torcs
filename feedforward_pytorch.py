...
class Layer:
    def __init__(self, neurons, inputs_per_neuron):
        self.weights = 2 * torch.randn((inputs_per_neuron, neurons)).type(dtype) - 1
        self.bias = 2 * torch.randn((neurons)).type(dtype) - 1
        self.activation = torch.zeros(neurons).type(dtype)
