...
class Layer:
    def __init__(self, neurons, inputs_per_neuron):
        self.weights = 0.0001 * (2.0 * np.random.random((inputs_per_neuron, neurons)) - 1.0)
        self.bias = 0.0001 * (2.0 * np.random.random((neurons)) - 1.0)
...
#            layer.activation = self.sigmoid(input.dot(layer.weights))
    ...
    def error(self, y, t, N, w_decay):
        ...
        return 1.0/(2.0 * N) * np.sum(np.square(y - t)) + w_decay / 2.0 * N * sum_w_sqrd
    
    # The derivative of the error function
    def derivative_error(self, y, t, N):
        return y - t
    ...
    def backward_propagation(self, x, y, t, L_rate, w_decay, N):
        ...
        output_error = self.derivative_error(y, t, N)
...
#            layer_delta = layer_error * self.sigmoid_derivative(self.layers[i].activation)
            layer_delta = np.multiply(layer_error, self.sigmoid_derivative(self.layers[i].activation))
#           
#           
            # Separate code for the first hidden layer after input, uses x
            # to calculate weight gradients, exit loop after.
            if i == 0:
                self.layers[i].weights *= 1 - L_rate * w_decay
        ...
        self.layers[-1].weights *= 1 - L_rate * w_decay
